...
const fragmentShader = () => {
    return `
        ...
        void main() {
            ...
            vec2 uv = pixelized;//rotate(pixelized, -time  * time_speed * .1);
            ...
            float n = fbm(circleUV*size -time * time_speed * .1);
            float nc = circle(circleUV*scale -time * time_speed * .1 + n);
...
export function createStarFlareLayer(rotationSpeed = 0.05, stormWidth = .2, stormDitherWidth = 0.07, colors, pixels, seed) {
    const palette = colors.scheme ? `src/colorScheme/starPalette/${colors.scheme}Palette.png` : "src/colorScheme/starPalette/orangePalette.png"
    ...
    const planetMaterial = new ShaderMaterial({
        uniforms: {
            pixels: { value: pixels ? (pixels * 2) : 200.0 },
            colorramp: { value: colorSchemeTexture },
            time_speed: { value: rotationSpeed },
            rotation: { value: Math.random() },
            seed: { value: seed || flip() ? Math.random() * 10 : Math.random() * 100 },
            time: { value: 0.0 },
