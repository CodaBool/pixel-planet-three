...
import { flip, hexToVector4 } from "../utils.js";
...
const fragmentShader = () => {
    return `
        ...
        float cloud_alpha(vec2 uv) {
            ...
            for (int i = 0; i < 9; i++) {
                c_noise += circleNoise((uv * size * 0.3) + (float(i+1)+10.) + (vec2(time*0.1*time_speed, 0.0)));
            }
            float fbm = fbm(uv*size+c_noise + vec2(time*0.1*time_speed, 0.0));
...
export function createGasPLayer(lightPos = new Vector2(0.39, 0.7), cloudCover = 0.538, colors, stretch = 1.0, rotationSpeed = 0.1, rotation = 0.0, cloudCurve = 1.3, pixels, seed) {
