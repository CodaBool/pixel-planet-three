...
const fragmentShaderRivers = () => {
    return `
        ...
        vec2 spherify_blend(vec2 uv, float blend) {
            vec2 centered = uv * 2.0 - 1.0;
            float z = sqrt(1.0 - dot(centered.xy, centered.xy));
            vec2 sphere = centered / (z + 1.0);
            sphere = sphere * 0.5 + 0.5;
            return mix(uv, sphere, blend); // blend between flat and spherical
        }
...
export function createRiverLayer(lightPos = new Vector2(0.39, 0.7), rotationSpeed = 0.1, rivers, colors, rotation = 0.0, pixels, seed) {
    ...
    return new Mesh(planetGeometryRivers, planetMaterialRivers);
}