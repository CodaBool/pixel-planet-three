...
const fragmentShaderPlanet = () => {
    return `
        ...
        void main() {
            ...
            uv = rotate(uv, time*time_speed * .4);
...
export const createAsteroid = (lightPos = new Vector2(0.39, 0.7), colors, rotation = 0.0, pixels, seed, size) => {
    ...
    // console.log("feature", colorPalette)

    // console.log(lightPos, colorPalette, rotation, pixels,seed,size)
    const planetGeometry = new PlaneGeometry(1.5, 1.5);
    const planetMaterial = new ShaderMaterial({
        uniforms: {
            pixels: {value: pixels || 100.0},
            size: {value: size || Math.random() * 10},
